...
const OTP: React.FC = () => {
  ...
  // Ensure useSearchParams is only accessed on the client side
  ...
  return (
    <div>
      ...
      <div className="flex justify-center items-start min-h-screen bg-cover bg-center pt-40" style={{ backgroundImage: `url(${bg})` }}>
        <div className="bg-black bg-opacity-70 p-8 rounded-lg max-w-md w-full">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            />
            {resentMsg && <h3>Resend OTP successful</h3>}
            <small>{errors.otp && <p className="text-red-500">{errors.otp.message}</p>}</small>
            {error && <h3 style={{ color: "red" }}>Invalid OTP</h3>}
            <br />
